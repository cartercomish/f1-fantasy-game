...
import { getTeamByIdAsync, LEAGUES, type LeagueId } from "@/lib/data";
...
export default async function LeagueTeamPage({
  ...
  const team = await getTeamByIdAsync(id, leagueId as LeagueId);
  if (!team) notFound();
  ...
  return (
    <div className="space-y-8">
      <Link
        ...
        className="text-[#8F8F9D] hover:text-white transition-colors text-sm"
      >
        ← Back to Dashboard
      </Link>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#8F8F9D] mt-1">
            Total: <span className="text-[#FF1E00] font-bold">{team.totalPoints} PTS</span>
          </p>
        </div>
      </div>
